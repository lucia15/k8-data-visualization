...
def xlsx_to_csv(xlsxfile, sheetname):
    ...
    if sheetname == 'Upwork People List' or sheetname == 'Offboarded People':
        ...
        data['Slack Channel'] = data['Slack Channel'].apply(
            lambda s: s.replace('#', '') if isinstance(s, str) else s)
            
        data['Slack Channel'] = data['Slack Channel'].apply(
            lambda s: '#' + s if isinstance(s, str) else s)

        data.dropna(subset=['Project Name'], inplace=True)

        data['Project Name'] = data['Project Name'].apply(
        ...
        # sort by priority
        data['Priority'] = pd.Categorical(
            data['Priority'], categories=[
                'One (1)', 'Two (2)', 'Three (3)'], ordered=True)
        ...
        data2 = data.copy()
        data2.drop(data2.columns.difference(
            ['Project Name', 'Slack Channel', 'Delivery Manager', 'Resource Name']), 1, inplace=True)
        ...
        data3.to_csv(
            d +
            'csv_files/Resource and responsability.csv',
            index=False,
            encoding='utf-8')
        #######################################################################

        data['Delivery Manager'] = data['Delivery Manager'].apply(
            lambda s: s.rstrip('\n').strip().replace(
                '\n', ', ') if isinstance(
                s, str) else s)
...
def csv_to_yaml(filename):
    ...
    :param filename: csv filename without the .csv, yaml file will be save with same name
    """
...
def get_between_parenthesis(mystring):
    ...
    Return a list with all strings between parenthesis.
